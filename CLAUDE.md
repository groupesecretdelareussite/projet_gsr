...
SQL migrations live in `sql/` and are applied manually to Supabase in numeric order (`001_schema_public.sql` → `018_td_liberer_creneau.sql` as of this writing); there is no migration runner. **`015`/`016`/`017` are applied on the client's Supabase (confirmed 2026-08-11) but NOT on the local test Supabase (`.env.local`)** — `td.classes_td` still exists there, so `npm run test:integration` fails and creating a TD créneau against the test DB breaks until someone applies the same files there too. **`018` (statut `Retiree`, `motif_retrait`, garde anti-double-booking) status not confirmed on either database** — verify before assuming it's live. See "Historique et points ouverts" below.
...
- **Export Excel** (2026-08-13) : `src/lib/export-excel.ts` + `src/components/admin/ExporterExcelButton.tsx`, génération client-side (`xlsx`), titre descriptif en ligne 1 de la feuille. Présent sur Élèves > Liste/Suspendus, Paiements > Historique/À jour/En retard, Comptabilité — gardé par rôle (`coordonnateur`/`comptable`/`superviseur`, `coordonnateur`/`comptable` seul pour Comptabilité), exporte exactement les lignes/filtres affichés à l'écran. Exclus volontairement : Notes/Moyennes (le client a un autre plan pour ça), Paiements > Supprimés, Statistiques (page de graphiques, pas de tableau ligne par ligne).
...
- **`sql/018_td_liberer_creneau.sql` écrit (2026-08-13) mais statut d'application non confirmé sur les deux bases** — tant que ce n'est pas fait, `libererCreneauTD()` échouera (colonne `motif_retrait`/statut `Retiree` absents) et `traiterArbitrageTD()` n'aura pas le nouveau garde-fou anti-double-booking.
- **`src/app/admin/eleves/suspendus/page.tsx` n'avait aucun contrôle de rôle** avant le 2026-08-13 (pas de `getUserScope`) — n'importe quel compte connecté pouvait voir tous les élèves suspendus de tous les sites. `getUserScope` a été ajouté uniquement pour gérer le bouton Export (voir ci-dessus) — le problème de fond (pas de filtrage par site pour un superviseur, pas de blocage pour chef_site/secrétaire) n'a pas été traité, à reprendre séparément si besoin.
